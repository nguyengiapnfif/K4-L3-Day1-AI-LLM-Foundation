...
def compare_models(prompt: str) -> dict:
    ...
    mini_res = call_openai_mini(prompt)

    dct = {"gpt4o_response": res[0], "mini_response": mini_res[0], "gpt4o_latency": res[1],
           "mini_latency": mini_res[1]}

    cost = (len(res[0].split()) / 0.75) / 1000
    dct["gpt4o_cost_estimate"] = cost * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
...
def chat_with_system_prompt(
    ...
    end_time = time.perf_counter()
    return res.choices[0].message.content, end_time - start_time
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    dct = {"input_tokens": count_tokens(prompt), "output_tokens": count_tokens(response), "input_cost": 0, "output_cost": 0, "total_cost": 0}

    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    dct["input_cost"] = dct["input_tokens"] / 1000 * pricing["input"]
    dct["output_cost"] = dct["output_tokens"] / 1000 * pricing["output"]
    dct["total_cost"] = dct["input_cost"] + dct["output_cost"]
...
def run_assistant(
    ...
    from openai import OpenAI            # import BÊN TRONG hàm để test mock được
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    num_turns, total_tokens, total_cost, history = 0, 0, 0.0, []
    ...
    while True:
        ...
        reply = ""
        for chunk in retry_with_backoff(lambda: client.chat.completions.create(model=OPENAI_MODEL, messages=messages, stream=True)):
            delta = chunk.choices[0].delta.content or ""
            reply += delta
            print(delta, end="", flush=True)
        num_turns += 1
